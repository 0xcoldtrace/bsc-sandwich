...
| — | `strategy-lock-mode2` (Chủ CHỐT chiến lược 2026-09-15) | XONG — CLAUDE.md mục "Chiến lược đã chốt" (4 quyết định: mode 2 only, vet tay, `sim_engine="v2"` hot path, revm giữ 3 việc nền/pre-sign/validator); `config.toml`/`config.rs` (`wallet_scan_enabled=false`, `sim_engine="v2"`, 2 field mới `pairs_vet_interval_sec`/`pairs_require_vetted`); `pairbook.rs` (`vetted_at`/`VetResult`/`vet_failed`/`tokens_to_vet`/gate ở `reload`); `main.rs::pairs_vet_task` (vet nền revm, không chặn hot path); `web.rs` `/api/pairs` thêm cột vet; `pairs.txt` 100 dòng đổi định dạng comment (`vetted` để trống — Chủ tự điền); `scripts/vet_goplus.sh` MỚI (lọc thô GoPlus, chạy thật 100 token, phát hiện 2 giới hạn hạ tầng GoPlus thật — xem docs/STATE.md); `scripts/paper_run.sh` hết ép `evm`/`universal`/`usdt`. Đổi tên roadmap: cụm 2 → `real-economics-mode2`, cụm 3 `fork-actor-perf` hạ ưu tiên xuống sau cụm 6 (lý do: hot path không còn fork EVM mỗi tx). `docs/VPS_RUN.md` đổi tên `docs/RUN.md`. Xem docs/STATE.md mục `strategy-lock-mode2` | BAOCAO36 |
